@extends('layouts.admin')
@section('content')
@can('host_country_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route("admin.host_country.create") }}">
                {{--{{ trans('global.add') }} {{ trans('cruds.sponsor.title_singular') }}--}}
                {{ trans('global.add') }} New
            </a>
        </div>
    </div>
@endcan
<div class="card">
    <div class="card-header">
        {{--{{ trans('cruds.sponsor.title_singular') }}--}}
        {{ trans('global.list') }}
    </div>

    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-Sponsor">
                <thead>
                    <tr>
                        <th width="10">

                        </th>
                        <th>
                            {{ trans('cruds.host_country.fields.id') }}
                        </th>

                        <th>
                            {{ trans('cruds.host_country.fields.category') }}
                        </th>
                        <th>
                            {{ trans('cruds.host_country.fields.name') }}
                        </th>
                        <th>
                            {{ trans('cruds.host_country.fields.logo') }}
                        </th>
                        <th>
                            {{ trans('cruds.host_country.fields.link') }}
                        </th>
                        <th>
                            &nbsp;
                        </th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($host_countries as $key => $host_country)
                        <tr data-entry-id="{{ $host_country->id }}">
                            <td>

                            </td>
                            <td>
                                {{ ++$key ?? '' }}
                            </td>
                            <td>
                                {{ $host_country->category ?? '' }}
                            </td>
                            <td>
                                {{ $host_country->name ?? '' }}
                            </td>
                            <td>
                                @if($host_country->logo)
                                    <a href="{{ $host_country->logo->getUrl() }}" target="_blank">
                                        <img src="{{ $host_country->logo->getUrl('thumb') }}" width="50px" height="50px">
                                    </a>
                                @endif
                            </td>
                            <td>
                                {{ $host_country->link ?? '' }}
                            </td>
                            <td>
                                @can('host_country_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.host_country.show', $host_country->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('host_country_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.host_country.edit', $host_country->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('host_country_delete')
                                    <form action="{{ route('admin.host_country.destroy', $host_country->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
                                        <input type="hidden" name="_method" value="DELETE">
                                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                        <input type="submit" class="btn btn-xs btn-danger" value="{{ trans('global.delete') }}">
                                    </form>
                                @endcan

                            </td>

                        </tr>
                    @endforeach
                </tbody>
            </table>
        </div>
    </div>
</div>
@endsection
@section('scripts')
@parent
<script>
    $(function () {
  let dtButtons = $.extend(true, [], $.fn.dataTable.defaults.buttons)
@can('host_country_delete')
  let deleteButtonTrans = '{{ trans('global.datatables.delete') }}'
  let deleteButton = {
    text: deleteButtonTrans,
    url: "{{ route('admin.host_country.massDestroy') }}",
    className: 'btn-danger',
    action: function (e, dt, node, config) {
      var ids = $.map(dt.rows({ selected: true }).nodes(), function (entry) {
          return $(entry).data('entry-id')
      });

      if (ids.length === 0) {
        alert('{{ trans('global.datatables.zero_selected') }}')

        return
      }

      if (confirm('{{ trans('global.areYouSure') }}')) {
        $.ajax({
          headers: {'x-csrf-token': _token},
          method: 'POST',
          url: config.url,
          data: { ids: ids, _method: 'DELETE' }})
          .done(function () { location.reload() })
      }
    }
  }
  dtButtons.push(deleteButton)
@endcan

  $.extend(true, $.fn.dataTable.defaults, {
    order: [[ 1, 'desc' ]],
    pageLength: 100,
  });
  $('.datatable-Sponsor:not(.ajaxTable)').DataTable({ buttons: dtButtons })
    $('a[data-toggle="tab"]').on('shown.bs.tab', function(e){
        $($.fn.dataTable.tables(true)).DataTable()
            .columns.adjust();
    });
})

</script>
@endsection