    ...
    <div class="card-body">
        <form action="{{ route("admin.schedules.update", [$schedule->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('subtitle') ? 'has-error' : '' }}">
                <label for="subtitle">{{ trans('cruds.schedule.fields.date') }}</label>
                <input type="text" id="subtitle" name="subtitle" class="form-control date" value="{{ old('subtitle', isset($schedule) ? $schedule->subtitle : '') }}">
